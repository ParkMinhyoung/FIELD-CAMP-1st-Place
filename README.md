...
**1) 최종 최적 입지 및 ESS 산출 결과 (총 budget 15조 원 / ESS budget 100억 원 기준)**
...
2. **CV 점수와 Test NMAE 간의 괴리 대응**
   ...
   - 겨울철 데이터 부재로 인해 단순 CV 점수에만 의존할 경우 오버피팅 발생 위험 확인. 실증 테스트 NMAE를 지속 점검하여 과도한 복잡도를 가진 v3 모델 대신 일반화 성능이 뛰어난 v2 모델을 선별 채택함.
